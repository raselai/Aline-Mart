...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchProduct = async (id: string) => {
    ...
    try {
      ...
      setCostPrice((product as any).costPrice?.toString() || '')
      setDiscountType(((product as any).discountType as 'percent' | 'flat') || 'percent')
      setDiscountValue((product as any).discountValue?.toString() || '')
      ...
      setShippingFeeInsideDhaka(product.shippingFeeInsideDhaka?.toString() || '')
      setShippingFeeOutsideDhaka(product.shippingFeeOutsideDhaka?.toString() || '')
      setWarranty(product.warranty || '')
      setVendor(product.vendor || '')
      setProductStatus(product.status === 'DRAFT' ? 'DRAFT' : 'ACTIVE')
      ...
      setVariants(
        (product.variants || []).map((variant, index) => ({
          ...
          color: variant.color || '',
          size: variant.size || '',
          sku: variant.sku || `default-${product.slug}-${index + 1}`,
          stock: variant.stock || 0,
          priceModifier: variant.priceModifier || 0,
          status: variant.stock && variant.stock > 0 ? 'in' : 'out',
        }))
      )
    } catch (error) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/products/${productId}`, {
        ...
        body: JSON.stringify({
          ...
          images: images.filter(img => img.url).map((img, index) => ({
            url: img.url,
            alt: img.alt || name,
            order: index
          })),
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 mx-auto mb-4" style={{ borderColor: '#8e2157' }}></div>
